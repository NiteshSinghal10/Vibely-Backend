...
import { getUserAdditionalInfo, updateUserAdditionalInfo } from '../../services';
...
router.get('/', async(req: Request, res: Response) => {
  try {
    const { sub } = req.user;
    const userInfo = await getUserAdditionalInfo({ _user: sub }) as IUserAdditionalInfo;
...
router.put('/interests', validateUpdateUserInterest, async(req: Request, res: Response) => {
  try {
    const { sub } = req.user;
    const { interests } = req.body;
    
    await updateUserAdditionalInfo({ _user: sub }, { interests }, { upsert: true });
